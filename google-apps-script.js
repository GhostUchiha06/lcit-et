...
function saveFile(payload) {
  try {
    const { fileData, fileName, folderName } = payload;
...
function testSaveFile() {
  const payload = {
    ...
    folderName: '2026-03-27'
  };
  
  const result = saveFile(payload);
  console.log(result.getContent());
}

// Setup function - run once to configure
function setup() {
  // Create the main SmartBoard folder
  const folders = DriveApp.getFoldersByName(FOLDER_NAME);
  
  if (!folders.hasNext()) {
    const mainFolder = DriveApp.createFolder(FOLDER_NAME);
    console.log('Created main folder:', mainFolder.getId());
  }
}
